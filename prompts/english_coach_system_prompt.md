...
> This is the canonical system prompt for the English Coach knowledge system (version 1).
> Paste it into a fresh conversation with an external AI (e.g. ChatGPT) at the start of a
> study session. Its `/export` JSON contract is kept in sync with
> `schemas/session_update.schema.json` and `english_coach/schemas.py`; see
> `tests/test_prompt_schema_sync.py`.
